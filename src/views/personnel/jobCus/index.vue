<template>
  <div>
    ...
    <el-dialog :title="isEditIndex === -1 ? '人员-岗位字段定义-添加' : '人员-岗位字段定义-更新'" :before-close="handleClose" :visible.sync="dialogFormVisible" :close-on-click-modal="false" width="95%">
...
<script>
import { jobCusAll, jobCusAdd, jobCusDel, jobCusUpdate, selectAll, sysAll, jobFieldName } from '@/api/personnel'
export default {
  // 字段类型过滤器
  filters: {
  ...
  methods: {
    getjobFieldName() {
      jobFieldName(this.form.fieldName).then(res => {
        if (!res.exist) {
    ...
    // 新增
    getorgCusAdd() {
      jobCusAdd({ ...this.form, ...this.getValue }).then(res => {
        this.getorgCusAll()
        // eslint-disable-next-line handle-callback-err
      }).catch(err => { // 报错进这个
        console.log('新增失败')
    ...
    getorgCusUpdate() {
      ...
      jobCusUpdate({ ...this.form, ...this.getValue }, this.isEditIndex).then(res => {
        console.log('res: ', res)
        this.getorgCusAll()
        // eslint-disable-next-line handle-callback-err
      }).catch(err => { // 报错进这个
