<template>
  <div>
    ...
    <el-dialog :title="isEditIndex === -1 ? '人员-组织字段定义-添加' : '人员-组织字段定义-更新'" :before-close="handleClose" :visible.sync="dialogFormVisible" :close-on-click-modal="false" width="95%">
...
<script>
import { orgCusAll, orgCusAdd, orgCusDel, orgCusUpdate, selectAll, sysAll, orgFieldName } from '@/api/personnel'
export default {
  // 字段类型过滤器
  filters: {
  ...
  methods: {
    getorgFieldName() {
      orgFieldName(this.form.fieldName).then(res => {
        if (!res.exist) {
    ...
    // 新增
    getorgCusAdd() {
      orgCusAdd({ ...this.form, ...this.getValue }).then(res => {
        this.getorgCusAll()
        // eslint-disable-next-line handle-callback-err
      }).catch(err => { // 报错进这个
        console.log('新增失败')
    ...
    getorgCusUpdate() {
      ...
      orgCusUpdate({ ...this.form, ...this.getValue }, this.isEditIndex).then(res => {
        console.log('res: ', res)
        this.getorgCusAll()
        // eslint-disable-next-line handle-callback-err
      }).catch(err => { // 报错进这个
