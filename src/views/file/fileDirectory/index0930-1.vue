...
<div>
    ...
    <div class="set_main">
        ...
        <div class="set_main_con">
            <el-row>
                ...
                <el-col>
                    <el-pagination style="clear: both" background :total="dataTotal" layout="prev, pager, next" :page-size="10" :current-page.sync="currentPage" @current-change="handleCurrentChange" />
                </el-col>
            </el-row>
        </div>
    </div>
    ...
    <!--共享弹窗 开始-->
    <el-dialog title="共享" :visible.sync="dialogFormVisible" :close-on-click-modal="false">
        <el-row style="margin-top: 10px; margin-right: 30px; text-align: right">
            <el-col :span="24">
                <el-button type="primary" @click="docPermissionSave">保存</el-button>
                <el-button>取消{{ shareData.type }}</el-button>
            </el-col>
        </el-row>
        <el-form :model="form" style="margin-right: 30px; margin-top: 20px">
            <el-form-item label="共享范围" label-width="120px">
                <el-select v-model="shareData.type" placeholder="请选择" style="width: 100%">
                    <el-option v-for="item in options" :key="item.value" :label="item.label" :value="item.value" />
                </el-select>
            </el-form-item>

            <el-form-item label="共享内容" label-width="120px" v-if="shareData.type !== '1'">
                <div v-if="shareData.type === '2'">
                <el-button  @click="selectOrg">选择部门</el-button>
                <div v-if="multipleSelectionOrg.length">
                    <el-tag v-for="tag in multipleSelectionOrg" :key="tag.id"  :type="tag.type">
                        {{ tag.name }}
                    </el-tag>
                </div>
                </div>
                ...
                <el-button @click="selectMen">选择人员</el-button>
                 <div v-if="multipleSelectionMen.length">
                    <el-tag v-for="tag in multipleSelectionMen" :key="tag.id"  :type="tag.type">
                        {{ tag.name }}
                    </el-tag>
                </div> 
                </div>
                ...
                <el-button @click="selectRole">选择角色</el-button>
                 <div v-if="multipleSelectionRole.length">
                    <el-tag v-for="tag in multipleSelectionRole" :key="tag.id"  :type="tag.type">
                        {{ tag.lanVal}}
                    </el-tag>
                </div>
                </div>
            ...
            <el-form-item label="安全级别" label-width="120px">
                <el-col :span="4">
                    <el-input v-model.number="shareData.startNo" autocomplete="off" type="number" oninput="value=value.replace(/[^\d.]/g,'')" :min="1" />
                </el-col>
                <el-col :span="2" style="text-align: center">-</el-col>
                <el-col :span="4">
                    <el-input v-model.number="shareData.endNo" autocomplete="off" type="number" oninput="value=value.replace(/[^\d.]/g,'')" :min="1" />
                </el-col>
            </el-form-item>
    ...
    <!--选择部门 开始-->
    <el-dialog title="选择部门" :visible.sync="dialogOrgVisible" :close-on-click-modal="false">
        <el-row>
        ...
        <el-table ref="multipleTable" :data="tableData1" tooltip-effect="dark" style="width: 100%" @selection-change="handleSelectionChangeOrg">
        ...
        <el-pagination background :total="dataTotal1" layout="prev, pager, next" :page-size="10" :current-page.sync="currentPage" @current-change="handleCurrentChangeOrg" />
    </el-dialog>
    <!--选择部门 结束-->

    <!--选择人员 开始-->
    <el-dialog title="选择人员" :visible.sync="dialogMenVisible" :close-on-click-modal="false">
        <el-row>
            <el-col :span="24" class="text-right">
                <el-button type="primary" @click="Menconfim">确认</el-button>
            </el-col>
        </el-row>
        <el-table ref="multipleTable" :data="tableData2" tooltip-effect="dark" style="width: 100%" @selection-change="handleSelectionChangeMen">
            <el-table-column type="selection" width="55"> </el-table-column>
        ...
        <el-pagination background :total="dataTotal2" layout="prev, pager, next" :page-size="10" :current-page.sync="currentPage" @current-change="handleCurrentChangeMen" />
    </el-dialog>
    <!--选择人员 结束-->

    <!--选择角色 开始-->
    <el-dialog title="选择人员" :visible.sync="dialogRoleVisible" :close-on-click-modal="false">
        <el-row>
            <el-col :span="24" class="text-right">
                <el-button type="primary"  @click="Roleconfim">确认</el-button>
            </el-col>
        </el-row>
        <el-table ref="multipleTable" :data="tableData3" tooltip-effect="dark" style="width: 100%" @selection-change="handleSelectionChangeRole">
            <el-table-column type="selection" width="55"> </el-table-column>
        ...
        <el-pagination background :total="dataTotal3" layout="prev, pager, next" :page-size="10" :current-page.sync="currentPage" @current-change="handleCurrentChangeRole" />
    </el-dialog>
    <!--选择角色 结束-->
</div>
</template>
...
export default {
    data() {
        return {
            ...
            shareData: {
                allJus: "0", //	删除权限
                content: "", //内容
                endNo: 0, //结束级别 截止安全级别,截止安全级别必须大于起始安全级别
                fileId: 0, //文件id
                modifyJus: "0", //修改权限
                readJus: "0", //读权限
    ...
    methods: {
        ...
        handleSelectionChangeOrg(val) {
            ...
            console.log("multipleSelectionOrg: ", this.multipleSelectionOrg);
        },
        //人员多选事件
        handleSelectionChangeMen(val) {
            this.multipleSelectionMen = val;
            console.log("multipleSelectionMen: ", this.multipleSelectionMen);
        },
